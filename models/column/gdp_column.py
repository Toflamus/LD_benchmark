...
def build_column(min_trays, max_trays, xD, xB):
    ...
    m.minimum_num_trays = Constraint(
        expr=sum(m.tray[t].indicator_var.get_associated_binary() for t in m.conditional_trays)
        + 1  # for feed tray
        >= min_trays
    )
    ...
    m.obj = Objective(
        ...
        + (sum(m.tray[t].indicator_var.get_associated_binary() for t in m.conditional_trays) + 1)
        * 1e3,  # [No. of Trays] to [$]
        sense=minimize,
    )
    ...
    def tray_ordering(m, t):
        ...
        if t + 1 < m.condens_tray and t > m.feed_tray:
            return m.tray[t].indicator_var.get_associated_binary() >= m.tray[t + 1].indicator_var.get_associated_binary()
        elif t > m.reboil_tray and t + 1 < m.feed_tray:
            return m.tray[t + 1].indicator_var.get_associated_binary() >= m.tray[t].indicator_var.get_associated_binary()
        else:
    ...
    @m.LogicalConstraint(m.conditional_trays)
    def link_YP_to_tray(m, n):
        return m.YP[n].equivalent_to(m.tray[n].indicator_var)
...
if __name__ == "__main__":
    ...
    SolverFactory("gdpopt.ldbd").solve(
            ...
            # 1. Starting Point 
            # ---------------------------------------------------------
            # The numbers here represent the index position in the "Select 1 of N" list (starting from 1).
            # The model has 15 internal trays (Tray 2 to Tray 16).
            # [13, 4] means:
            #   - The first logical constraint (one_reflux) selects the 13th candidate -> corresponds to Tray 14
            #   - The second logical constraint (one_boilup) selects the 4th candidate  -> corresponds to Tray 5
            starting_point=[15, 1], 

            # ---------------------------------------------------------
            # 2. Logical Constraints 
            # ---------------------------------------------------------
            # Tell the algorithm which constraints define the discrete variables.
            # In gdp_column.py, these two constraints are:
            #   - m.one_reflux: exactly(1, m.YR) restricts reflux location
            #   - m.one_boilup: exactly(1, m.YB) restricts boilup location
    ...
    # # Get the Pyomo logger and set its level to ERROR
    # # This will only show actual errors and suppress warnings
    # logging.getLogger('pyomo.core').setLevel(logging.ERROR)
    # logging.getLogger('pyomo.repn').setLevel(logging.ERROR)
    # logging.getLogger('pyomo.solvers').setLevel(logging.ERROR)
    # # Or simply suppress all Pyomo warnings:
    # logging.getLogger('pyomo').setLevel(logging.ERROR)
    
    # # ---------------------------------------------------------
    # # 1. Define Logic Transfer Function (Required)
    # # ---------------------------------------------------------
    # # The purpose of this function is: After fixing YR (Reflux location) and YB (Boilup location),
    # # explicitly calculate and fix whether each tray m.tray[n] exists or not.
    # # If this is not written, fix_disjuncts will raise an error because it cannot find values for indicator variables.
    # def problem_logic_column(m):
    #     logic_expr = []
        
    #     # Iterate through all conditional trays
    #     for n in m.conditional_trays:
    #         # Logic formula: Determine if tray n should exist
    #         # Existence condition (logic_val): (Reflux at n or below) AND ((Boilup above n) OR (Boilup is exactly at n))
    #         # This matches the logic of YP in the model
            
    #         # Part 1: Reflux location is at n or below (Indices: n ... max_trays-1)
    #         reflux_below_or_at = pe.lor(m.YR[j] for j in range(n, m.max_trays))
            
    #         # Part 2: Boilup location is before n (Indices: n ... max_trays-1 YB does not exist) ...
    #         # Or boilup location is exactly n
    #         boilup_above = pe.land(~m.YB[j] for j in range(n, m.max_trays))
    #         boilup_at = m.YB[n]
    #         boilup_condition = pe.lor(boilup_above, boilup_at)
            
    #         # Combined logic
    #         tray_active_condition = pe.land(reflux_below_or_at, boilup_condition)
            
    #         # Add to list: [Logic expression, Corresponding indicator variable]
    #         # Rule A: Condition met -> Tray exists
    #         logic_expr.append([tray_active_condition, m.tray[n].indicator_var])
            
    #         # Rule B: Condition not met -> Tray does not exist (Activate no_tray)
    #         logic_expr.append([~tray_active_condition, m.no_tray[n].indicator_var])
            
    #     return logic_expr

    # # ---------------------------------------------------------
    # # 2. Prepare Enumeration
    # # ---------------------------------------------------------
    # NT = 17
    # model_args = {
    #     'min_trays': 8,
    ...
    # # Create a temporary model to get variable references
    # m_temp = build_column(**model_args)
    
    # # Define external variable dictionary: Tell the algorithm which variables to enumerate
    # # We want to enumerate all combinations of YR (Reflux) and YB (Boilup) on the intTrays set
    # ext_ref = {
    #     m_temp.YR: m_temp.intTrays,
    #     m_temp.YB: m_temp.intTrays
    # }
    ...
    #     model_function=build_column,    # Pass build function
    #     model_args=model_args,          # Model arguments
    #     ext_dict=ext_ref,               # External variable definition
    #     ext_logic=problem_logic_column, # Logic transfer function
    #     mip_transformation=False,       # Use GDP form
    #     transformation='bigm',          # Transformation method
    #     feasible_model='column',        # Result filename prefix
        
    #     # Solver configuration (NLP)
    #     subproblem_solver='gams',
    #     subproblem_solver_options=dict(
    #         solver='dicopt',            # Recommended to use DICOPT (handles binary vars automatically)
    #         add_options=[
    #             "option reslim=100;",   # Time limit
    #             "option optcr=0.001;"
    #         ],
    #         keepfiles=True
    #     ),
    ...
    #     iter_timelimit=100,
    #     timelimit=None,                 # No total time limit, run until completion
    #     gams_output=False,
    #     # tee=True,                     # Show subproblem logs
    #     global_tee=True,                # Show enumeration progress
